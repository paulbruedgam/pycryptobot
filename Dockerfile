...
RUN export DEBIAN_FRONTEND=noninteractive && \
    ...
    apt-get install --assume-yes --no-install-recommends build-essential=12.9 python3-dev=3.9.2-3 \
    gfortran=4:10.2.1-1 libblas-dev=3.9.0-3 liblapack-dev=3.9.0-3 libjpeg62-turbo-dev=1:2.0.6-4 && \
    apt-get clean && \
...
RUN export DEBIAN_FRONTEND=noninteractive && \
    apt-get update && \
    apt-get upgrade --assume-yes && \
    apt-get install --assume-yes --no-install-recommends libatlas3-base=3.10.3-10 libjpeg62-turbo=1:2.0.6-4 && \
    apt-get clean && \
    rm -rf /var/lib/apt/lists/* && \
    groupadd -g 1000 pycryptobot && \
    useradd -r -u 1000 -g pycryptobot pycryptobot && \ 
    mkdir -p /app/.config/matplotlib && chown -R pycryptobot:pycryptobot /app
